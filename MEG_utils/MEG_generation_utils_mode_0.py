...
def crop_center_rectangle(image, focus_width, focus_height, ratio_left = 0.5, ratio_top = 0.5):
    """
    Crop out the central rectangular area of the image
    
    @ params
    - image: PIL Image (input)
    - focus_width, focus_height: width and height of the center region
    ...
    # Calculate the coordinates of the upper-left and lower-right corners of the center area
    ...
    # crop and return center region
    return image.crop(crop_coords), crop_coords
...
def create_latent_with_center_rectangle(latent_width, latent_height, focus_latent_width, focus_latent_height, focus_latent):
    ...
    top_left_x = (latent_width - focus_latent_width) // 2
    top_left_y = (latent_height - focus_latent_height) # // 2
    partial_latent[:,:,top_left_y:top_left_y+focus_latent_height,top_left_x:top_left_x+focus_latent_width] = focus_latent
...
def ES_generation(x, ES_MODEL, pipe, prompt, focus_width, focus_height, image_width, image_height, seed=111, create_full_image=False):
    ...
    # partial_image = create_image_with_center_rectangle(image_width, image_height, focus_image, focus_coords)
    # focus_image.save(os.path.join(SAVE_DIR, "focus_image.png"))  # save
    # partial_image.save(os.path.join(SAVE_DIR, "partial_generated_image.png")) 
    
    """ First create the full image and then split """
    ...
    if x > 0.7:
        ratio_left, ratio_top = 0.5, 0.8
    else:
        ratio_left, ratio_top = 0.5, 0.3
    # create the focus image
    focus_image, focus_coords = crop_center_rectangle(full_image, focus_width, focus_height, ratio_left, ratio_top)
    patial_image = create_image_with_center_rectangle(image_width, image_height, focus_image, focus_coords)
    # save and display the generated image
    focus_image.save(os.path.join(SAVE_DIR, "focus_image.png"))  # save
    patial_image.save(os.path.join(SAVE_DIR, f"partial_generated_image_{x}.png"))
...
def Mobile_Generation(splitting_ratio, UE_MODEL, pipe, prompt, W, H, focus_width, focus_height, focus_coords, UNET_MODEL=None, center_image=None):
    ...
    if UE_MODEL == 'sdxl-union':
        ...
        guidance_scale = 1.4 if splitting_ratio > 0.7 else 1.5
        seed = 1
        with torch.no_grad():
